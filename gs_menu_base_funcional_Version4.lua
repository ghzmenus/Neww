-- ============================================================================
-- GS MENU V4 - Interface Atualizada com Grid Layout 2 Colunas
-- Sistema de Toggles com Nova Estrutura Visual
-- ============================================================================
...
local tabContents = {
	HOME = {
		items = {"NOCLIP", "FLY", "BREV", "BREV", "BREV", "BREV"}
	},
	COMBATE = {
		items = {"AIMBOT", "SHOWFV", "WALLCK", "BREV", "BREV", "BREV"}
	},
	VISUAL = {
		items = {"ESP NAME", "ESP LIFE", "ESP TRACE", "ESP BOX", "ESP HEAD", "STAFF LIST"}
	},
	CAR = {
		items = {"ESP CAR", "FLY CAR", "NOCLIP", "BREV", "BREV", "BREV"}
	},
	TROLL = {
		items = {"NOWALL", "PUXAR TODOS", "BREV", "BREV", "BREV", "BREV"}
	}
}
...
local SCRIPT_URLS = {
	...
	SHOWFV = {
		on = function() print("[GS MENU] SHOWFV - EM BREVE") end,
		off = function() print("[GS MENU] SHOWFV - EM BREVE") end
	},
	WALLCK = {
	...
	["PUXAR TODOS"] = {
		on = function() print("[GS MENU] PUXAR TODOS - EM BREVE") end,
		off = function() print("[GS MENU] PUXAR TODOS - EM BREVE") end
	}
}
...
-- ============================================================================
-- CRIAR HEADER / CREATE HEADER
-- ============================================================================

local headerBar = Instance.new("Frame")
headerBar.Name = "HeaderBar"
headerBar.Size = UDim2.new(0.75, 0, 0, 50)
headerBar.Position = UDim2.new(0.125, 0, 0.05, 0)
...
titleText.Text = "GS MENU [sintonia rp]"
...
statusText.Parent = headerBar

-- ============================================================================
-- MENU ICON (HAMBURGER)
-- ============================================================================

local menuIconContainer = Instance.new("Frame")
menuIconContainer.Name = "MenuIcon"
menuIconContainer.Size = UDim2.new(0, 40, 0, 40)
menuIconContainer.Position = UDim2.new(0.93, 0, 0.5, -20)
menuIconContainer.BackgroundTransparency = 1
menuIconContainer.BorderSizePixel = 0
menuIconContainer.Parent = headerBar

-- Linha 1
local line1 = Instance.new("Frame")
line1.Size = UDim2.new(0, 25, 0, 2)
line1.Position = UDim2.new(0, 7, 0, 8)
line1.BackgroundColor3 = COLOR_WHITE
line1.BorderSizePixel = 0
line1.Parent = menuIconContainer

-- Linha 2
local line2 = Instance.new("Frame")
line2.Size = UDim2.new(0, 25, 0, 2)
line2.Position = UDim2.new(0, 7, 0, 19)
line2.BackgroundColor3 = COLOR_WHITE
line2.BorderSizePixel = 0
line2.Parent = menuIconContainer

-- Linha 3
local line3 = Instance.new("Frame")
line3.Size = UDim2.new(0, 25, 0, 2)
line3.Position = UDim2.new(0, 7, 0, 30)
line3.BackgroundColor3 = COLOR_WHITE
line3.BorderSizePixel = 0
line3.Parent = menuIconContainer
...
mainContent.Size = UDim2.new(0.75, 0, 0.75, 0)
mainContent.Position = UDim2.new(0.125, 0, 0.1, 50)
mainContent.BackgroundColor3 = COLOR_BACKGROUND
...
mainContentStroke.Color = COLOR_NEON_PURPLE
mainContentStroke.Thickness = 2
mainContentStroke.Transparency = 0
mainContentStroke.LineJoinMode = Enum.LineJoinMode.Round
mainContentStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
mainContentStroke.Parent = mainContent

-- ============================================================================
-- CRIAR TABS SIDEBAR / CREATE TABS SIDEBAR
-- ============================================================================

local tabsSidebar = Instance.new("Frame")
tabsSidebar.Name = "TabsSidebar"
tabsSidebar.Size = UDim2.new(0.15, 0, 1, 0)
tabsSidebar.Position = UDim2.new(0, 0, 0, 0)
tabsSidebar.BackgroundTransparency = 1
tabsSidebar.BorderSizePixel = 0
tabsSidebar.Parent = mainContent
...
dividerLine.Position = UDim2.new(0.15, -1, 0, 0)
dividerLine.BackgroundColor3 = COLOR_NEON_PURPLE
dividerLine.BorderSizePixel = 0
dividerLine.Parent = mainContent
...
contentArea.Size = UDim2.new(0.85, 0, 1, 0)
contentArea.Position = UDim2.new(0.15, 0, 0, 0)
...
local function createToggle(parent, name, index)
	local toggleContainer = Instance.new("Frame")
	toggleContainer.Name = name .. "Container"
	toggleContainer.Size = UDim2.new(1, 0, 0, 35)
	toggleContainer.Position = UDim2.new(0, 0, 0, (index - 1) * 40)
	...
	toggleButton.Position = UDim2.new(0.6, 0, 0.5, -12)
...
local function createTabContent(tabName)
	...
	paddingFrame.Size = UDim2.new(0.95, 0, 0.95, 0)
	paddingFrame.Position = UDim2.new(0.025, 0, 0.025, 0)
	paddingFrame.BackgroundTransparency = 1
	paddingFrame.BorderSizePixel = 0
	paddingFrame.Parent = tabContent
	...
	gridContainer.Parent = paddingFrame
	...
	-- Create toggles in 2-column layout
	local tabData = tabContents[tabName]
	if tabData then
		local toggleIndex = 1
		for i, toggleName in ipairs(tabData.items) do
			if i % 2 == 1 then
				createToggle(leftColumn, toggleName, math.ceil(i / 2))
			else
				createToggle(rightColumn, toggleName, math.ceil(i / 2))
			end
		end
	end
...
local function createTab(tabName, index)
	...
	indicator.Position = UDim2.new(-0.1, 0, 0.2, 0)
	...
	tabButton.MouseEnter:Connect(function()
		tabButton.TextColor3 = COLOR_NEON_PURPLE
	end)
	...
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		...
		dragState.dragInput = input.Changed:Connect(function()
			if dragState.isDragging then
				...
				mainContent.Position = dragState.startPos + UDim2.new(0, delta.X, 0, delta.Y + 50)
			end
		end)
	end
end)
...
print("[GS MENU] ✓ Menu V4 inicializado com sucesso!")
print("[GS MENU] ✓ Nova interface com grid 2-colunas ativada!")
print("[GS MENU] ✓ Abas: HOME, COMBATE, VISUAL, CAR, TROLL")
